...
## Recap.
Lets start out with a quick recap of some of the functions and idea from last lecture. 
...
x1
# The distribution represents the possible data set consisting of 20000 observations, with each observation having 10 trials, and the probability for each trial is 1/6.  
```
...
# The distribution represents the probability density function. It represents the probability of getting a certain number of successes for the values 0 to 10, with 10 trials and the probability of success for each trial is 1/6. 
```
...
# To estimate the difference, I subtract the two expected values. 
difference <- 10-5
difference

# 2. Estimate the uncertainty surrounding the point estimate difference.
# To estimate the uncertainty, I use the formula for the standard error of the difference. 
uncertainty <- sqrt((1.5^2)+(2^2))
uncertainty
...
## Some functions: 
### Sample
If I have an underlying idea about how many people eat the most candy on Friday or Saturday, we can use that to simulate the distribution. The sample function allows the outcome of sampling to be strings. 

```{r}
sample(c("friday","saturday"), size = 10, replace = TRUE, prob = c(0.55,0.45))
```
Replace = TRUE is required if you want a list with occurrences. Try and change it to false and see what happens. 
...
sample(c("Friday", "Saturday", "Sunday"), size = 10, replace = TRUE, prob = c(0.45, 0.35, 0.20))
...
```
```{r}
# repeat the number 1 and 2 one hundred times. 
rep(c(1,2), 100)
```
...
# Firstly, it calculates the sum of the vector which is 6. Secondly, the number 6 is divided by each number in the vector one at a time. This is repeated 100 times. 

```

### Loop()
If you want to change something in each iteration of the repeat then loops() is a good intuitive starting points. R is super good at working with vectorized data and operations but loops is sometimes easier to understand but also computationally more expensive. 
...
# first two terms
n1 = 0 # defines first two numbers 
n2 = 1
count = 2
...
if(nterms == 1) { 
print("Fibonacci sequence:") # prints first number (0)
print(n1) 
} else {
print("Fibonacci sequence:")
print(n1)
print(n2)
while(count < nterms) { #as long as the count is less that the amount of terms, the loop will continue
nth = n1 + n2 # defines the next number, the first and second number (0, 1 is equal to 1)
print(nth)
# update values
n1 = n2 
n2 = nth # calculates the next number based on the previous two (1+1 = 2 and so on)
count = count + 1 
}
}
}
...
A hypothetical study has shown that left handed has a 60% chance of having an IQ above 160 
...
This is a point estimate but there is of course some uncertainty which could be calculated using the 
...
If we repeated this procedure many times we could approximate the sampling distribution of our expected value which 
we can use to say something about the uncertainty of our point estimates.
